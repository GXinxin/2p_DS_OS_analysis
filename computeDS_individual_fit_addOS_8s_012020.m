...
for f = [1:6, 12:16, 19:25]% : size(fileInfo, 1)
    f
    compute_individual_DS(fileInfo{f, 1}, fileInfo{f, 2}, fileInfo{f, 3}, fileInfo{f, 4}, is_filter, 100000);
end


% compute DSI, OSI, tuning width, etc...
function compute_individual_DS(data_path, data_group, data_tag, rightEye, is_filter, shuffle_n)
...
if length(a_list) >= 1
    for a_id = 1 : length(a_list)
        ...
        if is_filter
            save_tag = ['tra2b_filtered_', num2str(detectWd), 's_zscore_'];
        else
        ...
        for ff = 1 : length(flist)
            cd(flist(ff).name)
            if is_filter
                load('responseTrace_noZscore_filtered.mat')
            else
        ...
        for t = 1 : 3
            ...
            for th = 1 : 2
                ...
                gDS_id{t}{th} = find(DSI_global(id1{t}, t) > thresh(th));
                DS_percent(th, t) = length(DS_id{t}{th}) / length(id1{t});
                gDS_percent(th, t) = length(gDS_id{t}{th}) / length(id1{t});
                DS_direction{t}{th} = directionId(id1{t}(DS_id{t}{th}), t); % was directionId(DS_id{t}{th}, t)
                gDS_direction{t}{th} = directionId(id1{t}(gDS_id{t}{th}), t); 
                if t == 2
                    all_direction{th} = respMatrix{t}(id1{t}(DS_id{t}{th}), :);
                    all_direction2{th} = respMatrix{t}(id1{t}(gDS_id{t}{th}), :);
                    weighted_direction{th} = [];
                    for ii = 1 : size(all_direction{th}, 1)
                        weighted_direction{th}(ii) = circ_mean((pi/4:pi/4:2*pi)', all_direction{th}(ii, :)') * 180/pi;
                    end
                end
                if t == 2 && th == 1
                    all_selectiveResponded = respMatrix{t}(id1{t}, :);
                end
                
                OS_id{t}{th} = find(OSI(id1{t}, t) > thresh(th));
                gOS_id{t}{th} = find(OSI_global(id1{t}, t) > 0.1);
                ...
                OS_only_id{t}{th} = setdiff(OS_id{t}{th}, DS_id{t}{th});
                OS_DS_id{t}{th} = intersect(OS_id{t}{th}, DS_id{t}{th});
                OS_only_percent(th, t) = length(OS_only_id{t}{th}) / length(id1{t});
                OS_DS_percent(th, t) = length(OS_DS_id{t}{th}) / length(id1{t});
                gOS_percent(th, t) = length(gOS_id{t}{th}) / length(id1{t});
                OS_only_direction{t}{th} = orientationId(id1{t}(OS_only_id{t}{th}), t); 
                OS_DS_direction{t}{th} = orientationId(id1{t}(OS_DS_id{t}{th}), t); 
                OS_DS_direction_direction{t}{th} = directionId(id1{t}(OS_DS_id{t}{th}), t); 
                gOS_direction{t}{th} = orientationId(id1{t}(gOS_id{t}{th}), t); 
                ...
                if t == 2
                    for cc = 1 : 4
                        ...
                        all_orientation{cc}{th} = respMatrix_axis{t}(id1{t}(idx), :);
                        all_orientation2{cc}{th} = respMatrix{t}(id1{t}(idx), :);
                        weighted_orientation{cc}{th} = [];
                        if ~isempty(idx)
                            for ii = 1 : size(all_orientation{cc}{th}, 1)
                                weighted_orientation{cc}{th}(ii, 1) = circ_mean((pi/4:pi/4:pi)', all_orientation{cc}{th}(ii, :)') * 180/pi;
                                tmp = circ_mean((pi/4:pi/4:2*pi)', all_orientation2{cc}{th}(ii, :)') * 180/pi;
                                tmp_id = find(tmp < 0);
                                tmp(tmp_id) = tmp(tmp_id) + 180;
                                weighted_orientation{cc}{th}(ii, 2) = tmp;
                            end
                        end
                    end   
                end
                ...
                fit_OS_only_id{t}{th} = setdiff(fit_OS_id{t}{th}, fit_DS_id{t}{th});
                fit_OS_DS_id{t}{th} = intersect(fit_OS_id{t}{th}, fit_DS_id{t}{th});
                fit_OS_only_percent(th, t) = length(fit_OS_only_id{t}{th}) / length(id2{t});
                fit_OS_DS_percent(th, t) = length(fit_OS_DS_id{t}{th}) / length(id2{t});
                
                fit_OS_only_direction{t}{th} = fit_results{t}(1, id2{t}(fit_OS_only_id{t}{th}));
                tmp = find(fit_OS_only_direction{t}{th} > 180);
                fit_OS_only_direction{t}{th}(tmp) = fit_OS_only_direction{t}{th}(tmp) - 180;
                tmp = find(fit_OS_only_direction{t}{th} <= 0);
                fit_OS_only_direction{t}{th}(tmp) = fit_OS_only_direction{t}{th}(tmp) + 180;
                
                fit_OS_DS_direction{t}{th} = fit_results{t}(1, id2{t}(fit_OS_DS_id{t}{th}));
                fit_OS_DS_direction_direction{t}{th} = fit_OS_DS_direction{t}{th};
                tmp = find(fit_OS_DS_direction{t}{th} > 180);
                fit_OS_DS_direction{t}{th}(tmp) = fit_OS_DS_direction{t}{th}(tmp) - 180;
                tmp = find(fit_OS_DS_direction{t}{th} <= 0);
                fit_OS_DS_direction{t}{th}(tmp) = fit_OS_DS_direction{t}{th}(tmp) + 180;
            end
        ...
        if is_filter
            save([save_folder, 'filtered_DSI_summary_window', num2str(detectWd), '.mat'], 'directionId', 'orientationId', 'DS_direction', 'anova_p', ...
                ...
                'dist_p', 'pref_diff', 'pair_id', 'pref_diff_range', 'pair_counts')
        else
            save([save_folder, 'DSI_summary_window', num2str(detectWd), 'addCorr.mat'], 'directionId', 'orientationId', 'DS_direction', 'anova_p', ...
            'median_values', 'DSI', 'OSI', 'DSI_fit', 'OSI_fit', 'DS_id', 'DS_percent', 'DS_direction', 'OS_id', 'OS_percent', 'OS_direction', 'fit_DS_id', 'fit_DS_percent', 'fit_DS_direction', ...
            'fit_OS_id', 'fit_OS_percent', 'fit_OS_direction', 'DSI_global', 'OSI_global', 'percent_inactive', 'DSI_fit', 'OSI_fit', 'is_respond', 'id1', 'id2',...
            'is_fit', 'is_selective', 'time2peak', 'time2slope', 'respMatrix', 'respMatrix_axis', 'tuning', 'fitCoeff', 'gof', 'fit_results', 'resp_Percent',...
            'dist_p', 'pref_diff', 'pair_id', 'corr_value', 'OS_only_id', 'OS_DS_id', 'OS_only_direction', 'OS_DS_direction', 'OS_DS_direction_direction', ...
            'weighted_orientation', 'fit_OS_only_id', 'fit_OS_DS_id', 'fit_OS_only_direction', 'fit_OS_DS_direction', 'fit_OS_DS_direction_direction',...
            'fit_OS_only_percent', 'fit_OS_DS_percent', 'OS_only_percent', 'OS_DS_percent', 'gOS_id', 'gDS_id', 'gOS_direction', 'gDS_direction',...
            'gOS_percent', 'gDS_percent', '-v7.3')

        % 'dist_p_new', 'pref_diff_new', 'pair_id_new', 'pref_diff_range_new', 'pair_counts_new'
        end
