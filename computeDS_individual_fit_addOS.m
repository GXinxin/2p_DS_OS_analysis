...
if length(a_list) >= 1
    for a_id = 1 : length(a_list)
        ...
        save_tag = ['individual_', num2str(detectWd), 's_zscore_'];
        ...
        % load responseTrace
        flist = dir(fullfile('responseTrace_noZscore.mat'));
        if isempty(flist)
            cd('acq1')
            flist = dir(fullfile('responseTrace_noZscore.mat'));
        end
...
%                 meanAmp{t}(:, :, d) = squeeze(mean(respTrace{d}(timeStep(t) : timeStep(t+1) - 1, :, :)));
        ...
        for t = 1 : 3
            ...
            for th = 1 : 2
                ...
                DS_direction{t}{th} = directionId(DS_id{t}{th}, t);
                
                OS_id{t}{th} = find(OSI(id1{t}, t) > thresh(th));
                OS_percent(th, t) = length(OS_id{t}{th}) / length(id1{t});
                OS_direction{t}{th} = orientationId(OS_id{t}{th}, t);
                ...
                fit_DS_direction{t}{th} = fit_results{t}(1, fit_DS_id{t}{th});
                ...
                fit_OS_direction{t}{th} = fit_results{t}(1, fit_OS_id{t}{th});
        ...
        save([save_folder, 'DSI_summary_window', num2str(detectWd), '.mat'], 'directionId', 'orientationId', 'DS_direction', 'anova_p', ...
            ...
            'is_fit', 'is_selective', 'time2peak', 'time2slope', 'respMatrix', 'respMatrix_axis', 'percent_inactive', 'tuning', 'fitCoeff', 'gof', 'fit_results')
