        ...
        if sum(is_include{g}(ff, :)) > 0
            ...
            for t = 1 : 3
                ...
                percent_OS_only_fit_animal{t, g}(:, a) = sum(percent_OS_only_fit{t, g}(:, select_fov) .* repmat(cell_no{t, g}(1, select_fov), 2, 1), 2) ...
                    ./ repmat(sum(cell_no{t, g}(1, select_fov)), 2, 1);
                select_Percent_animal{t, g}(a) = sum(cell_no{t, g}(3, select_fov)) / sum(cell_no{t, g}(2, select_fov));
...
isSingleTh = 1;
range = [0.8, 0.5, 0.6, 0.3, 0.5, 0.5, 0.5, 0.5, 0.5, 1, 1];
...
for cc = 1 : 11
    ...
    for t = 1 : 3
        ...
        for g = 1:length(tag)
            switch cc
                ...
                case 9
                    ...
                    cc_tag = 'OS_only_fit';
                case 10
                    data = select_Percent_animal{t, g};
                    cc_tag = 'selective_respond';
                case 11
                    data = select_Percent_animal{t, g} ./ resp_Percent_animal{t, g};
        ...
        if cc == 1 || cc > 9
            [p2{1}(t, cc), hh2{1}(t, cc)] = ranksum(tmp{1}, tmp{2});
            p2{2}(t, cc) = NaN;
            hh2{2}(t, cc) = NaN;
        else
            [p2{1}(t, cc), hh2{1}(t, cc)] = ranksum(tmp{1}, tmp{2});
            [p2{2}(t, cc), hh2{2}(t, cc)] = ranksum(tmp{3}, tmp{4});
        end
    end
end
...
range = [0.6, 0.6, 0.35, 0.4, 0.6, 0.3];
for cc = 1 : 6
...
%         [p3(t, cc), hh3(t, cc)] = ttest2(tmp{1}, tmp{2});
        [p11(t, cc), hh11(t, cc)] = ranksum(tmp{1}, tmp{2});
        if t == 2   
            [cc mean(tmp{1}) std(tmp{1})/sqrt(length(tmp{1})) mean(tmp{2}) std(tmp{2})/sqrt(length(tmp{2}))]
        end
        title([cc_tag, ' t', num2str(t), ' ', num2str(p11(t, cc))])
    end
...
% save([save_tag, 'stats_', fn_tmp{is_smooth+1}, '_th', num2str(ii), '_animal.mat'], 'hh0', 'hh1', 'hh2', 'hh3', 'hh4', 'hh5', 'is_include')
% 
% 
% 
% 
