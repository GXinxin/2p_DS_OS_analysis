        ...
        for g = 1:length(tag)
            switch cc
                ...
                case 9
                    ...
                    cc_tag = 'OS_only_fit';
                case 10
                    data = select_Percent_all{t, g};
                    cc_tag = 'selective_respond';
                case 11
                    data = select_Percent_all{t, g} ./ resp_Percent_all{t, g};
                    cc_tag = 'selective_in_respond';
            end
...
for th = 1 : 2
    for g = 1 : 2
        for ii = 1 : length(DS_direction_individual{2, th, g})
            % DS
            for d = 1 : 8
                direction_count_n{th}{1}(ii, d, g) = sum(DS_direction_individual{2, th, g}{ii} == d) / length(DS_direction_individual{2, th, g}{ii});
            end

            % DS fit
            data = DS_direction_individual_fit{2, 1, g}{ii};
            tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
            direction_count_n{th}{2}(ii, :, g) = tmp_h.BinCounts / length(data);

            % OS
            data = [OS_direction_individual{2, th, g}{ii}; OS_direction_individual{2, th, g}{ii} + 4];
            for d = 1 : 8
                direction_count_n{th}{3}(ii, d, g) = sum(data == d) / length(data);
            end

            % OS fit
            data = [OS_direction_individual_fit{2, th, g}{ii}; OS_direction_individual_fit{2, th, g}{ii} + 180];
            tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
            direction_count_n{th}{4}(ii, :, g) = tmp_h.BinCounts / length(data);

            % OSDS
            data = [OS_DS_direction_individual{2, th, g}{ii}; OS_DS_direction_individual{2, th, g}{ii} + 4];
            for d = 1 : 8
                direction_count_n{th}{5}(ii, d, g) = sum(data == d) / length(data);
            end

            % OSDS_direction
            data = OS_DS_direction_direction_total_individual{2, th, g}{ii};
            for d = 1 : 8
                direction_count_n{th}{6}(ii, d, g) = sum(data == d) / length(data);
            end

            % OS_only
            data = [OS_only_direction_total_individual{2, th, g}{ii}; OS_only_direction_total_individual{2, th, g}{ii} + 4];
            for d = 1 : 8
                direction_count_n{th}{7}(ii, d, g) = sum(data == d) / length(data);
            end

            % OSDS fit
            data = [OS_DS_direction_total_individual_fit{2, th, g}{ii}; OS_DS_direction_total_individual_fit{2, th, g}{ii} + 180];
            tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
            direction_count_n{th}{8}(ii, :, g) = tmp_h.BinCounts / length(data);

            % OS_only fit
            data = [OS_only_direction_total_individual_fit{2, th, g}{ii}; OS_only_direction_total_individual_fit{2, th, g}{ii} + 180];
            tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
            direction_count_n{th}{9}(ii, :, g) = tmp_h.BinCounts / length(data);

            % OS fit
            data = OS_DS_direction_direction_total_individual_fit{2, th, g}{ii};
            tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
            direction_count_n{th}{10}(ii, :, g) = tmp_h.BinCounts / length(data);
        ...
        for ff = 1 : length(is_include{g})
            if sum(is_include{g}(ff, :)) > 0
                ...
                if length(select_fov) == 2
                    data = [DS_direction_individual{2, th, g}{select_fov(1)}; DS_direction_individual{2, th, g}{select_fov(2)}];
                elseif length(select_fov) == 1
                    data = DS_direction_individual{2, th, g}{select_fov(1)};
                end
                for d = 1 : 8
                    direction_count_n_animal{th}{1}(a, d, g) = sum(data == d) / length(data);
                end


                % DS fit
                if length(select_fov) == 2
                    data = [DS_direction_individual_fit{2, th, g}{select_fov(1)}, DS_direction_individual_fit{2, th, g}{select_fov(2)}];
                elseif length(select_fov) == 1
                    data = DS_direction_individual_fit{2, th, g}{select_fov};
                end
                tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
                direction_count_n_animal{th}{2}(a, :, g) = tmp_h.BinCounts / length(data);


                % OS
                if length(select_fov) == 2
                    data = [OS_direction_individual{2, th, g}{select_fov(1)}; OS_direction_individual{2, th, g}{select_fov(1)} + 4; ...
                        OS_direction_individual{2, th, g}{select_fov(2)}; OS_direction_individual{2, th, g}{select_fov(2)} + 4];
                elseif length(select_fov) == 1
                    data = [OS_direction_individual{2, th, g}{select_fov}; OS_direction_individual{2, th, g}{select_fov} + 4];
                end
                for d = 1 : 8
                    direction_count_n_animal{th}{3}(a, d, g) = sum(data == d) / length(data);
                end


                % OS fit
                if length(select_fov) == 2
                    data = [OS_direction_individual_fit{2, th, g}{select_fov(1)}, OS_direction_individual_fit{2, th, g}{select_fov(1)} + 180, ...
                        OS_direction_individual_fit{2, th, g}{select_fov(2)}, OS_direction_individual_fit{2, th, g}{select_fov(2)} + 180];
                elseif length(select_fov) == 1
                    data = [OS_direction_individual_fit{2, th, g}{select_fov}, OS_direction_individual_fit{2, th, g}{select_fov} + 180];
                end
                tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
                direction_count_n_animal{th}{4}(a, :, g) = tmp_h.BinCounts / length(data);


                % OSDS
                if length(select_fov) == 2
                    data = [OS_DS_direction_individual{2, th, g}{select_fov(1)}; OS_DS_direction_individual{2, th, g}{select_fov(1)} + 4; ...
                        OS_DS_direction_individual{2, th, g}{select_fov(2)}; OS_DS_direction_individual{2, th, g}{select_fov(2)} + 4];
                elseif length(select_fov) == 1
                    data = [OS_DS_direction_individual{2, th, g}{select_fov}; OS_DS_direction_individual{2, th, g}{select_fov} + 4];
                end
                for d = 1 : 8
                    direction_count_n_animal{th}{5}(a, d, g) = sum(data == d) / length(data);
                end


                % OSDS_direction
                if length(select_fov) == 2
                    data = [OS_DS_direction_direction_total_individual{2, th, g}{select_fov(1)}; OS_DS_direction_direction_total_individual{2, th, g}{select_fov(2)}];
                elseif length(select_fov) == 1
                    data = OS_DS_direction_direction_total_individual{2, th, g}{select_fov(1)};
                end
                for d = 1 : 8
                    direction_count_n_animal{th}{6}(a, d, g) = sum(data == d) / length(data);
                end


                % OS_only
                if length(select_fov) == 2
                    data = [OS_only_direction_total_individual{2, th, g}{select_fov(1)}; OS_only_direction_total_individual{2, th, g}{select_fov(1)} + 4; ...
                        OS_only_direction_total_individual{2, th, g}{select_fov(2)}; OS_only_direction_total_individual{2, th, g}{select_fov(2)} + 4];
                elseif length(select_fov) == 1
                    data = [OS_only_direction_total_individual{2, th, g}{select_fov}; OS_only_direction_total_individual{2, th, g}{select_fov} + 4];
                end
                for d = 1 : 8
                    direction_count_n_animal{th}{7}(a, d, g) = sum(data == d) / length(data);
                end


                % OSDS fit
                if length(select_fov) == 2
                    data = [OS_DS_direction_total_individual_fit{2, th, g}{select_fov(1)}, OS_DS_direction_total_individual_fit{2, th, g}{select_fov(1)} + 180, ...
                        OS_DS_direction_total_individual_fit{2, th, g}{select_fov(2)}, OS_DS_direction_total_individual_fit{2, th, g}{select_fov(2)} + 180];
                elseif length(select_fov) == 1
                    data = [OS_DS_direction_total_individual_fit{2, th, g}{select_fov}, OS_DS_direction_total_individual_fit{2, th, g}{select_fov} + 180];
                end
                tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
                direction_count_n_animal{th}{8}(a, :, g) = tmp_h.BinCounts / length(data);


                % OS_only fit
                if length(select_fov) == 2
                    data = [OS_only_direction_total_individual_fit{2, th, g}{select_fov(1)}, OS_only_direction_total_individual_fit{2, th, g}{select_fov(1)} + 180, ...
                        OS_only_direction_total_individual_fit{2, th, g}{select_fov(2)}, OS_only_direction_total_individual_fit{2, th, g}{select_fov(2)} + 180];
                elseif length(select_fov) == 1
                    data = [OS_only_direction_total_individual_fit{2, th, g}{select_fov}, OS_only_direction_total_individual_fit{2, th, g}{select_fov} + 180];
                end
                tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
                direction_count_n_animal{th}{9}(a, :, g) = tmp_h.BinCounts / length(data);


                % OSDS direction fit
                if length(select_fov) == 2
                    data = [OS_DS_direction_direction_total_individual_fit{2, th, g}{select_fov(1)}, OS_DS_direction_direction_total_individual_fit{2, th, g}{select_fov(2)}];
                elseif length(select_fov) == 1
                    data = OS_DS_direction_direction_total_individual_fit{2, th, g}{select_fov};
                end
                tmp_h = polarhistogram(deg2rad(data), 'BinEdges', -1/8*pi : 1/4*pi : 15/8*pi);
                direction_count_n_animal{th}{10}(a, :, g) = tmp_h.BinCounts / length(data);
    ...
    for cc = 1:10%[3:5, 7:9] 
        ...
        for th = 1:2
            ...
            for g = 1:length(tag)
                ...
                if isanimal
                    individual_data{g} = direction_count_n_animal{th}{cc}(:, :, g);
                else
                    individual_data{g} = direction_count_n{th}{cc}(:, :, g);
                end
            ...
            hold off
            saveas(h, ['1_noSmooth\rose_wsem\', num2str(cc), cc_tag, '_rose_t2only_wsem_animal', num2str(isanimal), '_th', num2str(th), '.png'])
            if isanimal
                ...
                [d_count_animal_c{cc}{th}, m] = multcompare(d_count_animal_stats{cc}{th}, 'dimension', [1 2]);
                ...
                [d_count_c{cc}{th}, m] = multcompare(d_count_stats{cc}{th}, 'dimension', [1 2]);
...
%                     fov_n(cc, g) = length(notnan_id);
                ...
                saveas(h, ['1_noSmooth\rose_wsem\', num2str(cc), cc_tag, '_hist_t2only_wsem_animal', num2str(isanimal), '_th', num2str(th), '.png'])
                ...
                if isanimal
                    ...
                    [o_count_animal_c{cc}{th}, m] = multcompare(o_count_animal_stats{cc}{th}, 'dimension', [1 2]);
                    ...
                    [o_count_c{cc}{th}, m] = multcompare(o_count_stats{cc}{th}, 'dimension', [1 2]);
...
save([save_tag, 'stats_', fn_tmp{is_smooth+1}, '_th', num2str(ii), '_FOV.mat'], 'hh1', 'hh2', 'hh3')
